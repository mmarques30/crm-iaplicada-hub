...
export function FunnelTab() {
  ...
  return (
    <>
          ...
          <Card key={card.label} className="border-border">
            <CardHeader className="pb-3">
              <CardTitle className="text-base flex items-center gap-2 text-foreground">
                <Users className="w-4 h-4 text-[#3A7BBF]" />
                {card.label}
              </CardTitle>
              <p className="text-2xl font-bold font-mono text-foreground">{card.total} <span className="text-sm font-normal text-muted-foreground">contatos</span></p>
            </CardHeader>
            <CardContent className="space-y-2">
              <div className="space-y-1">
                {[
                  { label: 'Leads', value: card.leads, color: '#1A8A6E' },
                  { label: 'Opportunities', value: card.opportunities, color: '#3A7BBF' },
                  { label: 'Customers', value: card.customers, color: '#6B8C22' },
                ].map(row => (
              ...
              <div className="pt-2 border-t border-border">
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span className="font-mono font-bold text-[#B87D2A]">{card.convRate.toFixed(1)}%</span>
                </div>
              </div>
              {card.topSources.length > 0 && (
                <div className="pt-2 border-t border-border space-y-1">
                  <p className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold">Sub-fontes</p>
                  {card.topSources.map(([name, count]) => (
                    <div key={name} className="flex items-center justify-between text-xs">
                      <span className="text-foreground/70 truncate max-w-[160px]">{humanizeCampaignName(name, 22)}</span>
      ...
      {/* Ecosystem Banner */}
      <Card className="border-[#6B8C22]/30 bg-[#F0F4E6]">
        <CardContent className="py-5">
          <div className="text-center space-y-2">
            <p className="text-xs uppercase tracking-widest text-[#6B8C22] font-semibold">TOTAL ECOSSISTEMA META/INSTAGRAM</p>
            <p className="text-3xl font-bold font-mono text-foreground">{ecosystemTotal} <span className="text-base font-normal text-muted-foreground">contatos ({ecosystemPct}% do total)</span></p>
            <div className="flex items-center justify-center gap-6 text-sm">
              <span><strong className="font-mono text-[#3A7BBF]">{ecosystemOpportunities}</strong> <span className="text-muted-foreground">Opportunities</span></span>
              <span><strong className="font-mono text-[#6B8C22]">{ecosystemCustomers}</strong> <span className="text-muted-foreground">Customers</span></span>
              <span><strong className="font-mono text-[#B87D2A]">{ecosystemDeals}</strong> <span className="text-muted-foreground">Deals</span></span>
            </div>
          </div>
        </CardContent>
      </Card>
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-[var(--c-raised)]">
                  <TableHead className="font-bold min-w-[140px] text-foreground">FONTE</TableHead>
                  {allColumns.map(col => (
                    <TableHead key={col} className={`text-center text-xs min-w-[70px] ${col === 'NEGÓCIO PERDIDO' ? 'text-[#C0392B]' : 'text-foreground/70'}`}>
                      {col === 'NEGÓCIO PERDIDO' ? 'PERDIDO' : col.length > 12 ? col.substring(0, 10) + '…' : col}
                    </TableHead>
                  ))}
                  <TableHead className="text-center font-bold text-xs text-foreground">TOTAL</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {crossTabSources.map(row => (
                  <TableRow key={row.source}>
                    <TableCell className="font-medium text-sm text-foreground">{row.source}</TableCell>
                    {allColumns.map(col => {
                      ...
                      return (
                        <TableCell key={col} className="text-center">
                          {val > 0 ? (
                            <span className={`inline-flex items-center justify-center w-7 h-7 rounded-full text-xs font-bold ${isLost ? 'bg-red-100 text-[#C0392B]' : 'bg-[#EFF5E0] text-[#4A6B14]'}`}>
                              {val}
                            </span>
                          ) : (
                    ...
                    <TableCell className="text-center font-mono font-bold text-foreground">{row.total}</TableCell>
                  </TableRow>
                ))}
