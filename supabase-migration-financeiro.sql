-- ============================================
-- MIGRAÇÃO: Módulo Financeiro Completo
-- Tabelas: parcelas, notas_fiscais, despesas, metas, repasses
-- ============================================
...
CREATE TABLE IF NOT EXISTS parcelas (
  ...
  tipo text NOT NULL DEFAULT 'parcela', -- 'entrada' | 'parcela'
...
CREATE TABLE IF NOT EXISTS notas_fiscais (
  ...
  valor numeric NOT NULL DEFAULT 0,
  status_nf text NOT NULL DEFAULT 'pendente', -- 'pendente' | 'emitida' | 'enviada'
  data_emissao date,
  data_envio date,
  created_at timestamptz DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS despesas (
  ...
  categoria text NOT NULL DEFAULT 'outros', -- 'folha' | 'sistemas' | 'publicidade' | 'custos' | 'outros'
  tipo text NOT NULL DEFAULT 'pontual', -- 'fixa' | 'pontual'
  status text NOT NULL DEFAULT 'pendente', -- 'pendente' | 'pago'
  pagamento text DEFAULT 'a_vista', -- 'a_vista' | 'parcelado'
  forma_pgto text, -- 'pix' | 'boleto' | 'cartao'
  valor numeric NOT NULL DEFAULT 0,
  created_at timestamptz DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS metas (
  ...
  categoria text NOT NULL, -- 'receita_total' | 'receita_business' | 'receita_academy' | 'receita_skills' | 'vendas_total' | 'folha' | 'publicidade' | 'custos' | 'sistemas'
...
-- View: Fluxo de caixa (parcelas pagas por mês)
...
-- View: Despesas por mês e categoria
